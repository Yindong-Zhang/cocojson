...
def convert(xml_files, image_dir, json_file):
    ...
    for i, xml_file in enumerate(xml_files):
        ...
        for obj in get(root, "object"):
            ...
            # assert xmax > xmin, f"xmax {xmax} must > xmin {xmin} in {xml_file}"
            # assert ymax > ymin, f"ymax {ymax} must > ymin {ymin} in {xml_file}"
            if xmax <= xmin:
                print(f"警告: xmax {xmax} <= xmin {xmin} in {xml_file}, discard bbox {ET.tostring(obj, encoding='unicode')}")
                continue
            if ymax <= ymin:
                print(f"警告: ymax {ymax} <= ymin {ymin} in {xml_file}, discard bbox {ET.tostring(obj, encoding='unicode')}")
                continue
